...
- **Habilitar Kernel**: Se você usa Linux, irá precisar executar o comando abaixo pra adicionar headers no seu Kernel. Caso você tenha instalado "*Docker Desktop*", no caso de ***macOS***, este passo não será necessário
...
O APKO é outra ferramenta da Chainguard, complementar ao Melange.
...
contents:
  ...
  repositories:
    ...
    - https://dl-cdn.alpinelinux.org/alpine/edge/main
    - https://dl-cdn.alpinelinux.org/alpine/edge/community
    - https://dl-cdn.alpinelinux.org/alpine/v3.15/main # SDKs .net
  packages:
    - bash
    - openssl
...
Neste caso temos um complicador, alguns pacotes estão em outras versões de repositórios, para que num Dockerfile simples seja instalado, seria necessário dezenas de ajustes teriam que ser realizados previamente(gambiarras). Logo a versão disponível no repostório do alpine é atrasada várias versões em relaçao aos releases da Microsoft. Logo temos que seguir a instalação padrão do latest do repostório que é a versão. 
...
RUN dotnet tool install dotnet-sonarscanner --global \
    && dotnet tool install dotnet-coverage --version 17.13.1 --global \
    && dotnet tool install Swashbuckle.AspNetCore.Cli --version 7.2.0 --global
```
